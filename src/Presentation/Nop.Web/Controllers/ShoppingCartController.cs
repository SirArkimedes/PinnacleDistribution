...
namespace Nop.Web.Controllers
{
    public partial class ShoppingCartController : BasePublicController
    {
        ...
        [HttpPost]
        public virtual IActionResult AddProductToCart_Details(int productId, int shoppingCartTypeId, int index, string btnName, IFormCollection form)
        {
            var product = _productService.GetProductById(productId);
            if (product == null)
            {
            ...
            // @JONAS I need to determine how to find out what button the user clicked
            // @JONAS I edited this form to find and update the cart with ajax and user forms. 
            //quantity
            var quantity = 1;
            foreach (var formKey in form.Keys)
                if (formKey.Equals($"EnteredQuantity", StringComparison.InvariantCultureIgnoreCase))
                {
                    string name = btnName.Replace("btn", "");

                    if (product.Id == Convert.ToInt32(name))
                    {
                        var results = form.ToList();

                        var supportedNums = results[1].Value.ToList();

                        quantity = Convert.ToInt32(supportedNums[index]);
                ...
                else if (formKey.Equals($"addtocart_{productId}.EnteredQuantity", StringComparison.InvariantCultureIgnoreCase))
                {
                    int.TryParse(form[formKey], out quantity);
